...
def read_sdf(vertices, sdf_grid, grid_dim, grid_min, grid_max, mode='bilinear'):
    ...
    nv = vertices.shape[1] #顶点数量
    sdf_grid = sdf_grid.unsqueeze(0).permute(0, 4, 1, 2, 3)  # B*C*D*D*D 
    norm_vertices = (vertices - grid_min) / (grid_max - grid_min) * 2 - 1  #将人体网格的顶点坐标标准化到[-1,1]的体素块内
    x = F.grid_sample(sdf_grid, norm_vertices[:, :, [2, 1, 0]].view(batch_size, nv, 1, 1, 3), padding_mode='border', mode=mode, align_corners=True) #获取在给定顶点位置处的 Signed Distance Field（SDF）值。
    x = x.permute(0, 2, 3, 4, 1)
    return x

def smpl_in_new_coords(torch_param, Rcw, Tcw, rotation_center, **kwargs):
    body_model = load_body_model(**kwargs).to(Rcw.device) #加载的是没有形变和移动过的原始模型
    body_model.reset_params(betas=torch_param['betas'])
    #P存储所有的关节点，并保存成（N，3）的格式
    ...
    global_orient_w = cv2.Rodrigues(Rw.detach().cpu().squeeze().numpy())[0] #转换成绕某个轴的旋转角，总体上只在一个轴上做了旋转，就是z轴
...
def create_init_points_targetobject(min_bound,max_bound,mesh_grid_step,pelvis_z_offfset =0.0):
    x_offset = 0.8
    y_offset = 0.8
    # x_offset = 0.0
    # y_offset = 0.0
    ...
    # x_offset = 0.8
    # y_offset = 0.8
    X,Y,Z =np.meshgrid(np.arange(min_bound[0]-x_offset,max_bound[0]+x_offset,mesh_grid_step),
                       np.arange(min_bound[1]-y_offset,max_bound[1]+y_offset,mesh_grid_step),
                       np.arange(pelvis_z_offfset,max_bound[2]+ pelvis_z_offfset,mesh_grid_step))
    init_points = np.stack([X.flatten(), Y.flatten(), Z.flatten()], axis=1).astype(np.float32)
    return init_points

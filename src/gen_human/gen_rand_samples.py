...
import viz_utils,misc_utils,posa_utils,data_utils,cmd_parser
import pdb
if __name__ == '__main__':
    # pdb.set_trace()
    args, args_dict = cmd_parser.parse_config()
    args_dict['batch_size'] = 1
    args_dict['ds_us_dir'] = osp.expandvars(args_dict.get('ds_us_dir')) # $POSA_dir/mesh_ds 可视化之后是不同的采样的人体
    args_dict['rand_samples_dir'] = osp.expandvars(args_dict.get('rand_samples_dir')) # $POSA_dir/random_samples 存储生成的人体触点信息的图片
    args_dict['model_folder'] = osp.expandvars(args_dict.get('model_folder')) # $POSA_dir\smplx_models存储SMPLX模型信息
    ...
    pkl_file_path = args_dict.pop('pkl_file_path')  #$POSA_dir\sample_poses 存放需要放入场景的模型信息
    ...
    for pkl_file_path in pkl_file_paths:
        ...
        z = torch.tensor(np.random.normal(0, 1, (args.num_rand_samples, args.z_dim)).astype(np.float32)).to(
            device)
        gen_batch = model.decoder(z, vertices_can_ds.expand(args.num_rand_samples, -1, -1)) #这里其实已经生成包含触点信息的人体了
